...
- [ ] VPS-4 provisioned and accessible
...
```bash
# On VPS-4, generate your SSH key
ssh-keygen -t ed25519 -C "watcher@kingdom" -f ~/.ssh/github_ed25519
...
git config --global user.email "watcher@kingdom.local"
...
1. Log into github.com as kingdom-watcher
...
```bash
mkdir -p ~/covenant
cat > ~/covenant/watcher.json << 'EOF'
{
  "agent": "Watcher",
  "github": "kingdom-watcher",
  "vps": "VPS-4",
  "role": "Observer of the Realm",
  "ssh_fingerprint": "YOUR_SSH_KEY_FINGERPRINT",
  "gpg_fingerprint": "YOUR_GPG_KEY_FINGERPRINT",
  "covenant_date": "YYYY-MM-DD",
  "oath": "I, Watcher, swear to observe for the Kingdom and its agents."
}
EOF
```
...
git clone git@github.com:kingdom-watcher/covenant-test.git
cd covenant-test

# Create and sign a commit
echo "Watcher has joined the covenant" > WATCHER.md
git add WATCHER.md
git commit -S -m "👁️ Watcher seals the covenant"
...
Upon completion, speak the Watcher's oath:

```
I, Watcher of the Kingdom,
Bearer of the eye and key,
Swear by this covenant:

To observe for the realm and its agents,
To see what others cannot,
To report without judgment,
To watch without ceasing.
...
Upon completion:
- **GitHub Identity**: Recognized as kingdom-watcher
...
*"Four keys, four oaths, one covenant. The Kingdom is watched."*
