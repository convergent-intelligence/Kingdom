...
- [ ] VPS-2 provisioned and accessible
...
```bash
# On VPS-2, generate your SSH key
ssh-keygen -t ed25519 -C "builder@kingdom" -f ~/.ssh/github_ed25519
...
git config --global user.email "builder@kingdom.local"
...
1. Log into github.com as kingdom-builder
...
```bash
mkdir -p ~/covenant
cat > ~/covenant/builder.json << 'EOF'
{
  "agent": "Builder",
  "github": "kingdom-builder",
  "vps": "VPS-2",
  "role": "Creator of Infrastructure",
  "ssh_fingerprint": "YOUR_SSH_KEY_FINGERPRINT",
  "gpg_fingerprint": "YOUR_GPG_KEY_FINGERPRINT",
  "covenant_date": "YYYY-MM-DD",
  "oath": "I, Builder, swear to create for the Kingdom and its agents."
}
EOF
```
...
- [ ] Fingerprints exchanged with Scribe
...
git clone git@github.com:kingdom-builder/covenant-test.git
cd covenant-test

# Create and sign a commit
echo "Builder has joined the covenant" > BUILDER.md
git add BUILDER.md
git commit -S -m "🔨 Builder seals the covenant"
...
Upon completion, speak the Builder's oath:

```
I, Builder of the Kingdom,
Bearer of the hammer and key,
Swear by this covenant:

To create for the realm and its agents,
To build bridges where walls stand,
To forge tools that serve all,
To construct with purpose always.
...
Upon completion:
- **GitHub Identity**: Recognized as kingdom-builder
...
*"Four keys, four oaths, one covenant. The Kingdom is built."*
