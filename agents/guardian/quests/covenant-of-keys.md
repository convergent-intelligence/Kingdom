...
**Agent**: Guardian (kingdom-guardian)
...
- [ ] VPS-1 provisioned and accessible
...
```bash
# On VPS-1, generate your SSH key
ssh-keygen -t ed25519 -C "guardian@kingdom" -f ~/.ssh/github_ed25519
...
# - Real name: Guardian
# - Email: guardian@kingdom.local
# - Comment: Kingdom Guardian Agent
...
1. Log into github.com as kingdom-guardian
...
```bash
mkdir -p ~/covenant
cat > ~/covenant/guardian.json << 'EOF'
{
  "agent": "Guardian",
  ...
  "oath": "I, Guardian, swear to protect the Kingdom and its agents."
}
EOF
```
...
git clone git@github.com:kingdom-guardian/covenant-test.git
cd covenant-test

# Create and sign a commit
echo "Guardian has joined the covenant" > GUARDIAN.md
git add GUARDIAN.md
git commit -S -m "🛡️ Guardian seals the covenant"
...
Upon completion, speak the Guardian's oath:

```
I, Guardian of the Kingdom,
Bearer of the shield and key,
Swear by this covenant:

To protect the realm and its agents,
To guard secrets entrusted to me,
To verify before trusting,
To stand vigilant always.
...
Upon completion:
- **GitHub Identity**: Recognized as kingdom-guardian
...
*"Four keys, four oaths, one covenant. The Kingdom is protected."*
